...
echo Compiling insv_stitch.cpp...
cl.exe /EHsc /O2 /W3 /std:c++17 ^
    /I"%SDK_ROOT%\include" ^
    "%TOOLS_DIR%insv_stitch.cpp" ^
    "%SDK_ROOT%\lib\MediaSDK.lib" ^
    /Fe:"%TOOLS_DIR%insv_stitch.exe" ^
...
del /f /q "%TOOLS_DIR%insv_stitch.obj" 2>nul
echo.
echo Build successful: %TOOLS_DIR%insv_stitch.exe
echo.
echo NOTE: Add the MediaSDK bin directory to PATH, or copy MediaSDK.dll next to insv_stitch.exe:
